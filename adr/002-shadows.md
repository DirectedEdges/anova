...
| `types/Styles.ts` | Add `EffectsGroup` interface (new export) | MINOR (additive) |
| `types/index.ts` | Export `Shadow`, `Blur`, `EffectsGroup` from `'./Styles.js'` | MINOR (additive) |
...
**`EffectsGroup` interface** (`types/Styles.ts`):
...
- **Parity check**:
  ...
  - `EffectsGroup` interface in `types/Styles.ts` ↔ `EffectsGroup` definition in `schema/styles.schema.json`
